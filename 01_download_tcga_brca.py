...
# =============================================================================
# Cell 5: Keep BOTH tumor and matched normal samples (real cancer-vs-normal task)
# TCGA sample barcodes: TCGA-XX-XXXX-01 = primary tumor, -11 = solid tissue normal
# =============================================================================
# CHANGED: previously this script kept ONLY tumor (-01) samples and discarded
# all normal (-11) samples, which meant the human side of the pipeline never
# had a real "no cancer" class - Step 5 had to fake a binary target out of
# PAM50 subtypes (aggressive vs non-aggressive), which is a different
# question than "is this cancer or not". Now we keep both sample types and
# build a genuine tumor/normal label, matching the canine side's real
# tumor/normal labels (see 02_download_canine.py).

sample_type_code = {c: c[13:15] for c in expr_human.columns}
tumor_cols  = [c for c in expr_human.columns if sample_type_code[c] == "01"]
normal_cols = [c for c in expr_human.columns if sample_type_code[c] == "11"]

print(f"\nTotal samples: {expr_human.shape[1]}")
print(f"Tumor samples (suffix -01): {len(tumor_cols)}")
print(f"Normal samples (suffix -11): {len(normal_cols)}")

keep_cols = tumor_cols + normal_cols
expr_human_tumor_normal = expr_human[keep_cols].copy()
print(f"Expression matrix after keeping tumor+normal: {expr_human_tumor_normal.shape}")

# Build the real tumor/normal label (this is now the PRIMARY human label
# used downstream in Step 4/5 - matches canine's real tumor/normal label)
tumor_normal_labels = pd.Series(
    ["tumor"] * len(tumor_cols) + ["normal"] * len(normal_cols),
    index=keep_cols,
    name="tumor_normal"
)
print(f"\nTumor/Normal label distribution:")
print(tumor_normal_labels.value_counts())
print("NOTE: TCGA-BRCA is naturally imbalanced here (~90% tumor, ~10% normal) -")
print("      keep this in mind for class-weighting/metric choice in Step 5.")

tumor_normal_labels.to_csv("data/human/TCGA_BRCA_tumor_normal_labels.csv")
print("Saved: data/human/TCGA_BRCA_tumor_normal_labels.csv")

# Save the combined expression matrix (renamed from "tumor_expression" since
# it now contains both tumor AND normal samples)
expr_human_tumor_normal.to_csv("data/human/TCGA_BRCA_tumor_normal_expression.csv")
print("Saved: data/human/TCGA_BRCA_tumor_normal_expression.csv")
print("\nStep 1 COMPLETE.")
